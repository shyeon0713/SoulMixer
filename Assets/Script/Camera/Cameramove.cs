...
public class Cameramove : MonoBehaviour
{
    ...
    public static Cameramove instance;  // 싱글톤 생성
    ...
    void Start()
    {
        ...
        if (Buttons.Count > 0)
        {
            ...
            Buttons[5].onClick.AddListener(MovePos2);  //pos4로 이동
            Buttons[6].onClick.AddListener(MovePos1);  //pos4로 이동
        }
    }
    ...
    #region - 카메라 위치를 변경할 경우, 어디서든 호출 가능-> Cameramove.instance.MoveToPosition();
    public void MoveToPosition(int index)
    {
        if(index < 0 || index >= CameraPosition.Count)
        {
            return;
        }

        this.transform.position = CameraPosition[index];
        this.transform.eulerAngles = CameraRotation[index];
    }
